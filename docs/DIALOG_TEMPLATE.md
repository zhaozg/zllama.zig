...
> 当前代码：`src/chat_template.zig`（Phase 1 硬编码模板）
...
| 单测覆盖（7 个测试） | ✅ 已完成 |
| GGUF `tokenizer.chat_template` 元数据读取 | ❌ 缺失 |
| Jinja 模板引擎 | ❌ 缺失 |
| `--chat-template` CLI 覆盖标志 | ❌ 缺失 |
| `--system-prompt` CLI 标志 | ❌ 缺失 |
| 工具调用模板（tool_use） | ❌ 缺失 |
| 输入标记（input marking）安全特性 | ❌ 缺失 |
| 聊天模式上下文持久化（`-c/--chat` 多轮） | ❌ 缺失（当前仅单轮） |
...
### Phase 1：GGUF 模板读取 + CLI 标志（优先级最高）
...
- 新增 `readChatTemplate(allocator, io, gguf_file) !?[]const u8` 方法
- 读取 GGUF metadata key `tokenizer.chat_template`（string 类型）
...
- 将现有硬编码模板实现保持，新增 `TemplateSource` 联合类型：

```zig
pub const TemplateSource = union(enum) {
    /// 从 GGUF 元数据读取的 Jinja 模板字符串
    gguf_builtin: []const u8,
    /// 用户 CLI 传入的模板名称（如 "chatml", "llama3", "gemma"）
...
- 修改 `applySingleTurn` / `applyMultiTurn` 签名，接受可选的 `TemplateSource`

#### 任务 1.3：CLI 标志

**文件**：`src/main.zig`, `src/simple_main.zig`

- 新增 `--chat-template <name>`：指定模板名称（如 `chatml`, `llama3`, `gemma`）
- 新增 `--system-prompt <text>`：指定系统提示词
- 新增 `--no-chat-template`：禁用模板，原始 prompt 透传
...
- `generate()` 中根据优先级选择模板源

#### 任务 1.5：聊天模式多轮上下文

**文件**：`src/main.zig`

- 当前 `-c/--chat` 模式每次调用 `generate()` 都是单轮
...
参考 llama.cpp 的 `llm_chat_detect_template()`：

```zig
pub fn detectTemplate(tmpl_src: []const u8) TemplateKind {
...
| 文件 | 变更类型 | 描述 |
|------|---------|------|
| `src/chat_template.zig` | 重构 | 模块入口：类型定义 + API 导出 |
| `src/chat_template/types.zig` | 新增 | ChatMessage, TemplateKind, TemplateSource |
| `src/chat_template/registry.zig` | 新增 | 预设模板注册与查找 |
| `src/chat_template/templates/chatml.zig` | 新增/迁移 | ChatML 模板实现 |
| `src/chat_template/templates/llama3.zig` | 新增/迁移 | Llama 3 模板实现 |
| `src/chat_template/templates/gemma.zig` | 新增/迁移 | Gemma 模板实现 |
| `src/chat_template/templates/mistral.zig` | 新增 | Mistral 模板实现 |
| `src/chat_template/templates/deepseek.zig` | 新增 | DeepSeek 模板实现 |
| `src/chat_template/jinja/lexer.zig` | 新增 | Jinja 词法分析（Phase 3） |
| `src/chat_template/jinja/parser.zig` | 新增 | Jinja 语法分析（Phase 3） |
| `src/chat_template/jinja/runtime.zig` | 新增 | Jinja 执行引擎（Phase 3） |
| `src/gguf.zig` | 修改 | 新增 `readChatTemplate()` |
| `src/main.zig` | 修改 | CLI 标志 + ChatSession |
| `src/simple_main.zig` | 修改 | CLI 标志 + 模板集成 |
| `build.zig` | 修改 | 注册 chat_template 子模块 |
