...
- [x] **采样率**：是否强制为 16kHz？重采样算法是否与 llama.cpp 一致？
- [x] **窗口与 FFT**：窗口长度 320，hop 160，零填充至 512，Hann 窗口是否 `periodic=True`？
- [x] **梅尔滤波器**：HTK 梅尔刻度，128 个滤波器，频率范围 0~8000 Hz，`mel_floor=1e-3`？
