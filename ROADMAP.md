...
| **Qwen 3.5 架构分析文档** | ✅ **已完成** | `QWEN.md` 详细分析 |
| 与 llama.cpp 对比测试 | ⬜ 待完成 | |
| 线性注意力层完整实现 | ✅ 已完成 | `forwardSSM()` 含完整 GDN 流程 |
| Metal 后端集成 | ⬜ 待完成 | |
| CUDA 后端集成 | ⬜ 待完成 | |
...
**目标：** 完整实现 Qwen 3.5 的混合注意力架构（全注意力 + GDN 线性注意力交替）。

- [x] 从 GGUF 元数据读取 SSM 参数（conv_kernel, state_size, group_count, dt_rank, inner_size）
- [x] 实现 `forwardSSM()` — 线性注意力层（含 Conv1d + GDN + 门控归一化）
- [x] 实现 `forwardFullAttention()` — 全注意力层（含 Q-gate、Q/K norm、MRoPE）
- [x] 实现 `isFullAttentionLayer()` — 基于 `full_attention_interval` 的层类型判断
- [x] 实现 SSM 状态管理（conv_state + ssm_state 每层独立维护）
- [x] 实现 `resetSSMStates()` — 增量解码时重置 SSM 状态
- [x] 端到端推理测试通过
- [x] 完成 `QWEN.md` 架构分析文档
...
**当前优先级：** Qwen 3.5 混合架构完整实现已完成。下一步可推进 P7/P8（GPU 后端）或 P9（性能优化）。
