...
const log = std.log.scoped(.tool_compare_mtmd_audio);
// ============================================================================
// 配置
// ============================================================================
...
pub const MtmdAudioComparator = struct {
    ...
    pub fn run(self: *MtmdAudioComparator, io: std.Io) !bool {
        ...
        if (mm_mgr.audio_encoder) |enc| {
            enc.saveDebugData(io, self.allocator, audio_graph);
        }
        ...
        try stdout_file.writeStreamingAll(io, "=== Multimodal Audio Comparison ===\n");
