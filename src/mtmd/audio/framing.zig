...
///
/// 处理步骤：
/// 1. 半因果左填充（pad_left = frame_length/2）
/// 2. 提取重叠帧
/// 3. 应用 Hann 窗口
/// 4. 零填充到 FFT 大小
///
...
pub fn frameAudio(
    ...
    for (0..frame_count.n_frames) |fi| {
        ...
        // 提取帧并应用 Hann 窗口
        const copy_len = @min(frame_size, frame_count.n_samples_padded - @as(u32, @intCast(start)));
        const copy_actual = @min(copy_len, params.frame_length);
        for (0..copy_actual) |j| {
