...
/// 获取 GGUF 值的类型标签字符串
fn getTypeTag(val: gguf.MetadataValue, allocator: std.mem.Allocator) []const u8 {
...
/// 格式化 GGUF 元数据值为可读字符串
fn formatMetadataValue(val: gguf.MetadataValue, allocator: std.mem.Allocator) ![]u8 {
...
fn printModelLoaderInfo(gguf_file: *const gguf.GGUFFile, file_size: u64, filename: []const u8) void {
    _ = file_size;
    // 打印 GGUF 版本信息
...
fn tensorDataTypeToString(dt: gguf.TensorDataType) []const u8 {
    return switch (dt) {
        .f32 => "f32",
        .f16 => "f16",
        .q4_0 => "q4_0",
...
fn fileTypeToString(ft: u32) []const u8 {
    return switch (ft) {
        0 => "F32",
        1 => "F16",
        2 => "Q4_0",
...
fn countTotalParams(gguf_file: *const gguf.GGUFFile) u64 {
    var total: u64 = 0;
    for (gguf_file.tensors.items) |tensor| {
        var n_elems: u64 = 1;
...
fn countTensorBytes(gguf_file: *const gguf.GGUFFile) u64 {
    var total: u64 = 0;
    for (gguf_file.tensors.items) |tensor| {
        var n_elems: u64 = 1;
        for (0..tensor.n_dims) |i| {
...
/// 从 GGUF 元数据读取模型参数信息
fn getModelParamU32(gguf_file: *const gguf.GGUFFile, key: []const u8) ?u32 {
    return gguf_file.getU32(key);
}

fn printTokenizerInfo(tok: *tokenizer.Tokenizer, gguf_file: *const gguf.GGUFFile) void {
    // 获取架构名称
...
fn findFimToken(tok: *const tokenizer.Tokenizer, gguf_file: *const gguf.GGUFFile, key: []const u8, text: []const u8) u32 {
    // 先尝试从 GGUF 元数据读取
...
fn getTokenText(tok: *const tokenizer.Tokenizer, id: u32) []const u8 {
...
fn isEogToken(tok: *const tokenizer.Tokenizer, id: u32) bool {
    const eog_names = [_][]const u8{
        "<|endoftext|>",
        "<|im_end|>",
        "<|fim_pad|>",
...
fn findLfToken(tok: *tokenizer.Tokenizer) u32 {
    // 对于 BPE 类型，通过 tokenize("\n") 查找
...
fn eogIdsContains(ids: []const u32, id: u32) bool {
    for (ids) |existing| {
...
pub fn main(init: std.process.Init) !void {
    ...
    // 打印模型加载信息
    printModelLoaderInfo(&gguf_file, file_size, args.model_path);

    // 打印分词器信息
    printTokenizerInfo(&tok, &gguf_file);
